...
class PerformerLM(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.image_proj = nn.Linear(image_dim, max_seq_len * dim) 
        self.sen_proj = nn.Linear(sen_dim, max_seq_len * dim)
    ...
    def forward(self, x, y,z, return_encodings = False, output_attentions = False, **kwargs):
        b, n, device = *x.shape, x.device
        assert n <= self.max_seq_len, f'sequence length {n} must be less than the max sequence length {self.max_seq_len}'

        x = self.token_emb(x)
        y = self.image_proj(y)
        y = y.view(-1, x.shape[1], x.shape[2])
        z = self.sen_proj(z)
        z = z.view(-1, x.shape[1], x.shape[2])
